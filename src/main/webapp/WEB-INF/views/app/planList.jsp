...
<script>
    function submitCon() {
        if (confirm("Czy napewno chcesz usunac plan?")) {
            return true;
        } else {
            return false;
        }
            ...
            <div class="dashboard-content border-dashed p-3 m-4 view-height">
                ...
                <div class="schedules-content">
                    <table class="table border-bottom">
                        ...
                        <c:forEach items="${planListSort}" var="plan">
                        <tr class="d-flex">
                            <td class="col-1">${plan.id}</td>
                            <td class="col-2">${plan.name}</td>
                            <td class="col-7">${plan.description}</td>
                            <td class="col-2 d-flex align-items-center justify-content-center flex-wrap">
                                <form method = "POST" action = "/app/plan/delete" id="deleteForm">
                                    <button type="submit" name="planId" value="${plan.id}" class="btn btn-danger rounded-0 text-light m-1" onclick="return submitCon()">Usuń</button>
                                </form>
